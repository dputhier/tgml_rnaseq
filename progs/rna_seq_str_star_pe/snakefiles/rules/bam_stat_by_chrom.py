...
rule bam_stat_by_chrom:
        ...
        params: script=script, python_path=python_path, chrom_list=config["chrom_list"]
        message: """--- Performing some stats about mapping (by chrom)."""
        threads: 1
        shell:  """
         {params.python_path} {params.script} -i {input} -o {output.o} -d  {output.d} -s {params.chrom_list} &> {output[0]}.log
                """
